...
category_name = f"On Screen Category {unique_suffix}"
product_name = f"On Screen Product {unique_suffix}"
sku_code = f"DEL-{unique_suffix % 10000}"

with sync_playwright() as p:
    # Launch in HEADED mode with slow motion (1.5s delay per step)
    ...
    # Add Category
    print(f"Creating unique Category: '{category_name}'...")
    ...
    # Add Product
    print(f"Creating unique Product: '{product_name}'...")
    ...
    print("Clicking Save Product...")
    page.click('button:has-text("Save")')
    page.wait_for_timeout(3000)
    
    # Deactivate the Product
    print(f"Deactivating product: '{product_name}'...")
    row = page.locator(f'tr:has-text("{product_name}")')
    deactivate_btn = row.locator('button:has-text("Deactivate")')
    deactivate_btn.click()
    page.wait_for_timeout(3000)
    
    # Activate the Product again
    print(f"Re-activating product: '{product_name}'...")
    row = page.locator(f'tr:has-text("{product_name}")')
    activate_btn = row.locator('button:has-text("Activate")')
    activate_btn.click()
    page.wait_for_timeout(3000)
    ...
    print("All tests completed successfully. Keeping browser open for 10 seconds for review...")
    time.sleep(10)
    browser.close()
